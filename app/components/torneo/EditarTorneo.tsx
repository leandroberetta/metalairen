...
export default function EditarTorneo(
    { onEditarTorneo, idGuardado, nombreGuardado, sedeGuardado, provinciaGuardado, fechaGuardado, oficialGuardado }: {
        idGuardado: number,
        nombreGuardado: string,
        sedeGuardado: string,
        provinciaGuardado: string,
        fechaGuardado: string,
        oficialGuardado: boolean,
        onEditarTorneo(
            id: number,
            nombre: string,
            sede: string,
    ...
    const [nombre, setNombre] = useState<string>(nombreGuardado);
    const [sede, setSede] = useState<string>(sedeGuardado);
    const [provincia, setProvincia] = useState<string>(provinciaGuardado);
    const [fecha, setFecha] = useState<string>(fechaGuardado);
    const [oficial, setOficial] = useState<boolean>(oficialGuardado);
    const [errors, setErrors] = useState<Record<string, string>>({});

    const handleEditarTorneo = async () => {
        if (!nombre) {
        ...
        if (Object.keys(errors).length === 0) {
            const {torneoId, error} = await onEditarTorneo(idGuardado, nombre, sede, provincia, fecha, oficial);
    ...
    return (
        <div className="p-4 pt-0">
            ...
            <button onClick={() => { handleEditarTorneo() }} className="focus:outline-none dark:bg-yellow-300 dark:hover:bg-yellow-400 font-medium rounded text-sm px-2.5 py-2.5 me-2 text-gray-800">Guardar</button>
        </div>
    );
}

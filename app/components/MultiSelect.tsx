...
const MultiCheckboxSelect = ({ options, label, parameter}: { options: { [id: string]: string }, label: string, parameter: string }) => {
  ...
  return (
    <div className="relative inline-block text-left dark:shadow-xl dark:shadow-gray-800 w-full" ref={dropdownRef}>
      <button
        ...
        className="inline-flex justify-between w-full px-4 py-2 text-sm font-medium text-gray-400 dark:bg-gray-700 rounded-lg border dark:border-gray-600"
      >
      ...
      <div
        className={`${isDropdownOpen ? 'block' : 'hidden'
          } absolute z-50 mt-2 dark:bg-gray-700 dark:text-white rounded-md shadow-xl ring-1 ring-black ring-opacity-5 focus:outline-none w-full`}
      >
        <ul className="py-1 overflow-y-auto text-sm text-white">
          { Object.entries(options).map(([id, value]) => (
            <li key={id} className="px-4 py-2 hover:bg-yellow-300">
